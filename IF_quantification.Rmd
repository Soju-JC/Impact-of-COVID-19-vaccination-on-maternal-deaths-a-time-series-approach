...
```{r, echo=TRUE,message=FALSE,warning =FALSE,error=FALSE, fig.width=10, fig.height=6, fig.align='center'}
# ====== 1) IRFs com mais réplicas e (opcional) GIRF ======
nahead <- 14   # ou 14, 21; escolha científica
boot_runs <- 1000

irf_opts <- list(response = "deaths",
                 n.ahead = nahead,
                 cumulative = TRUE,
                 boot = TRUE,
                 runs = boot_runs,
...
# Cholesky (sensível à ordem)
irf_br_d1  <- do.call(irf, c(list(var1, impulse = "first_dose.BR",  ortho = TRUE ), irf_opts))
irf_br_d2  <- do.call(irf, c(list(var1, impulse = "immunized.BR",    ortho = TRUE ), irf_opts))
irf_ges_d1 <- do.call(irf, c(list(var1, impulse = "first_dose.GES",  ortho = TRUE ), irf_opts))
irf_ges_d2 <- do.call(irf, c(list(var1, impulse = "immunized.GES",   ortho = TRUE ), irf_opts))

# ====== 2) Funções utilitárias p/ converter e resumir IRF ======
to_percent <- function(x) 100*(exp(x) - 1)  # conversão exata
# para respostas pequenas, 100*x é boa aproximação; use a exata acima

summarize_irf <- function(irf_obj, resp_name = "deaths"){
  # extrai vetores
  m <- irf_obj$irf[[1]][, resp_name]
  lo <- irf_obj$Lower[[1]][, resp_name]
  hi <- irf_obj$Upper[[1]][, resp_name]
  h  <- seq_along(m) - 1
  ...
  # pico (mais negativo) e quando ocorre
  idx_min <- which.min(mP)
  peak    <- mP[idx_min]
  t_peak  <- h[idx_min]

  # janelas de significância (onde IC não cruza zero)
  sig <- (loP > 0) | (hiP < 0)
  sig_h <- h[sig]
  first_sig <- ifelse(length(sig_h)==0, NA, min(sig_h))
  last_sig  <- ifelse(length(sig_h)==0, NA, max(sig_h))

  # acumulado até H (área sob curva) — soma simples em %
  H <- max(h)
  cum_eff <- sum(mP)

  # meia-vida (metade do pico, em módulo). Procura retorno a 0.5*|peak|
  half_target <- 0.5*abs(peak)
  # ponto mais próximo depois do pico:
  after <- which(h > t_peak)
  half_life <- NA
  if (length(after)) {
    idx <- after[which.min(abs(abs(mP[after]) - half_target))]
    half_life <- h[idx]
  }
  ...
  tibble::tibble(
    ...
    peak_percent = peak,
    time_to_peak = t_peak,
    first_sig_h  = first_sig,
    last_sig_h   = last_sig,
    cum_effect_percent = cum_eff,
    half_life_h  = half_life
  )
}

lab <- function(x) switch(x,
  "first_dose.BR"  = "Dose 1 - Brasil",
  "immunized.BR"   = "Imunizados - Brasil",
  "first_dose.GES" = "Dose 1 - Gestantes/puérperas",
  "immunized.GES"  = "Imunizadas - Gestantes/puérperas",
  x)

sum_tbl <- dplyr::bind_rows(
  summarize_irf(irf_br_d1)  |> dplyr::mutate(impulse = lab("first_dose.BR")),
  summarize_irf(irf_br_d2)  |> dplyr::mutate(impulse = lab("immunized.BR")),
  summarize_irf(irf_ges_d1) |> dplyr::mutate(impulse = lab("first_dose.GES")),
  summarize_irf(irf_ges_d2) |> dplyr::mutate(impulse = lab("immunized.GES"))
) |> dplyr::relocate(impulse)

knitr::kable(sum_tbl, digits = 2, caption = "Resumo quantitativo das IRFs (percentuais).")

# ====== 3) Tabela ponto-a-ponto (% e ICs) para comparação objetiva ======
extract_irf_df <- function(irf_obj, impulse_label){
  m  <- irf_obj$irf[[1]][, "deaths"]
  lo <- irf_obj$Lower[[1]][, "deaths"]
  hi <- irf_obj$Upper[[1]][, "deaths"]
  h  <- seq_along(m) - 1
  tibble::tibble(
    impulse = impulse_label,
    h = h,
    mean_pct = to_percent(m),
    lo_pct   = to_percent(lo),
    hi_pct   = to_percent(hi),
    sig_95   = (lo_pct > 0) | (hi_pct < 0)
  )
}

irf_df <- dplyr::bind_rows(
  extract_irf_df(irf_br_d1,  lab("first_dose.BR")),
  extract_irf_df(irf_br_d2,  lab("immunized.BR")),
  extract_irf_df(irf_ges_d1, lab("first_dose.GES")),
  extract_irf_df(irf_ges_d2, lab("immunized.GES"))
)

# Exemplo: comparar, no horizonte de 14 dias, quem tem maior efeito acumulado
comp14 <- irf_df |> dplyr::filter(h <= 14) |>
  dplyr::group_by(impulse) |>
  dplyr::summarise(cum_0_14 = sum(mean_pct), .groups = "drop") |>
  dplyr::arrange(cum_0_14)

knitr::kable(comp14, digits = 2, caption = "Efeito acumulado (0–14 dias) em %.")

# ====== 4) FEVD: importância relativa de cada série na variância dos ônus de previsão ======
fe <- vars::fevd(var1, n.ahead = nahead)
# 'fe$deaths' é uma matriz nahead x K com percentuais por variável explicativa
fevd_deaths <- as.data.frame(fe$deaths)
fevd_deaths$h <- 0:(nrow(fevd_deaths)-1)
fevd_long <- tidyr::pivot_longer(fevd_deaths, -h, names_to = "variable", values_to = "percent")
knitr::kable(head(fevd_long, 12), digits = 1, caption = "FEVD para 'deaths' (primeiros horizontes).")
```


```{r, echo=TRUE,message=FALSE,warning =FALSE,error=FALSE, fig.width=10, fig.height=6, fig.align='center'}
# Pacotes necessários
loadlibrary("sandwich")
loadlibrary("lmtest")

local_projection <- function(y, shock, controls, H = 28, lags = 2){
  # y, shock: vetores alinhados (do df.diff3)
  # controls: matriz/data.frame com demais variáveis
  # Retorna tibble com beta(h), se(h), t, p
  out <- vector("list", H+1)
  Tn <- length(y)

  # monta matriz de regressores com defasagens
  mk_lags <- function(x, p){
    do.call(cbind, lapply(0:p, function(L) dplyr::lag(x, L)))
  }
  ...
  Xc <- as.data.frame(controls)
  names(Xc) <- paste0("x", seq_len(ncol(Xc)))
  big <- dplyr::bind_cols(df, Xc)

  for (h in 0:H){
    big$y_lead <- dplyr::lead(big$y, h)
    Z <- cbind(
      mk_lags(big$shock, lags),
      do.call(cbind, lapply(Xc, mk_lags, p = lags))
    )
    Z <- Z[(1+lags):(Tn-h), , drop = FALSE]
    yH <- big$y_lead[(1+lags):(Tn-h)]
    # regressão
    fit <- lm(yH ~ Z)
    # coef do shock contemporâneo (primeira coluna de Z)
    bh <- coef(fit)[2]
    vc <- sandwich::NeweyWest(fit, lag = 7, prewhite = FALSE, adjust = TRUE)
    se <- sqrt(diag(vc))[2]
    tval <- bh/se; pval <- 2*pt(-abs(tval), df = stats::df.residual(fit))
    out[[h+1]] <- data.frame(h = h, beta = bh, se = se, t = tval, p = pval)
  }
  dplyr::bind_rows(out)
}

# Exemplo (usando suas transformações em df.diff3):
y_deaths <- df.diff3[, "deaths"]
shock_ges_d1 <- df.diff3[, "first_dose.GES"]
controls <- as.matrix(df.diff3[, c("first_dose.BR","immunized.BR","immunized.GES")])

lp <- local_projection(y_deaths, shock_ges_d1, controls, H = 28, lags = 2)
lp$beta_pct <- to_percent(lp$beta)  # converter para %
knitr::kable(lp[1:10, c("h","beta_pct","p")], digits = 3,
             caption = "Local Projections: efeito por horizonte (%, p-valor).")
```
